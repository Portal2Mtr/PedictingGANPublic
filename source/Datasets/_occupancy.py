...
Managing class for the Occupancy dataset.
...
class OccupancyDataset(DatasetBase):

    def __init__(self):
        super(OccupancyDataset, self).__init__(majority_label=0.0, minority_label=1.0, synth_label=2)
        file_config = './config/dataconfig/occupancyconfig.yml'
    ...
    def load_data(self):
        ...
        trainframe = read_csv(trainfile, header=0)
        testframe = read_csv(testfile, header=0)
        dataframe = concat([trainframe, testframe], ignore_index=True)

        # get the values
        values = dataframe.values
        data, labels = values[:, 2:7], values[:, 7]
        ...
        for idx, sample in enumerate(data):
            for jdx, feature in enumerate(sample):
                if jdx in self.category_columns:
                    data[idx, jdx] = float(data[idx, jdx]) + numpy.random.uniform(0, uniform_mag)
