...
class BitcoinLate2012SubDataset(DatasetBase):

    def __init__(self,trainSplit=None):
        super(BitcoinLate2012SubDataset, self).__init__()
        file_config = './config/dataconfig/bitcoinlate2012subconfig.yml'
        ...
        self.num_gen_array = []
        self.load_idn = self.config['dataconfig']['has_idn']
        self.load_data()
    ...
    def load_data(self):
        ...
        """
        fileload = self.config['fileconfig']['dataFile']
        header = 1

        if self.load_idn:
            fileload = self.config['fileconfig']['idnFile']
            header = 0
        dataframe = read_csv(fileload, sep=',',header=header)
        # get the values
