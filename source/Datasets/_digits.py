...
Managing class for the Digits multiclass dataset.

"""


# Digits dataset pytorch class
import sys
import copy
...
class DigitsDataset(DatasetBase):
    ...
    def __init__(self,trainSplit=None):
        ...
        file_config = './config/dataconfig/digitsconfig.yml'
    ...
    def load_data(self):
        ...
        self.config['dataconfig']['trainperc'] = (len(trainframe))/(len(trainframe) + len(testframe))

        dataframe = concat([trainframe, testframe], ignore_index=True)
        dataframe = np.array(dataframe)
        data, labels = dataframe[:, :-1], dataframe[:, -1]
        ...
        data_train = [sample for sample in data[0:len(trainframe)]]
        y_train = [label for label in labels[0:len(trainframe)]]
        data_test = [sample for sample in data[len(trainframe):]]
        y_test = [label for label in labels[len(trainframe):]]
