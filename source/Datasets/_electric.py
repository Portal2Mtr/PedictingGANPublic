...
# Electric stability dataset pytorch class
import sys
import copy
...
class ElectricDataset(DatasetBase):

    def __init__(self):
        super(ElectricDataset, self).__init__(majority_label=0.0, minority_label=1.0, synth_label=2)
        file_config = './config/dataconfig/electricconfig.yml'
    ...
    def load_data(self):
        ...
        for idx, label in enumerate(labels):
            if label == 'unstable': # unstable as majority
                labels[idx] = 0
            else:
                labels[idx] = 1
