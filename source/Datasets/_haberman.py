...
class HabermanDataset(DatasetBase):

    def __init__(self,trainSplit=None):
        super(HabermanDataset, self).__init__()
        file_config = './config/dataconfig/habermanconfig.yml'
    ...
    def load_data(self):
        ...
        self.train_idxs = [i for i in range(len(all_data))]

        for sample, label in zip(data_test, y_test):
            all_data.append(sample)
            all_labels.append(label)

        self.test_idxs = [i for i in range(self.train_idxs[-1], len(all_data))]
    ...
    def append_train_test_synth(self, train_data, test_data, aug_name=None):
        ...
        Args:
            train_data (): Synthetic training data tuple with tensors
            test_data (): Synthetic testing data with possible tensors
